...
####[GitHub repository](https://github.com/RyanEckert/Belize_Mcav_Symbiodiniaceae_ITS2-ASV.git){target="_blank"}
***
#About this document
***
####All analyses preformed with R version `r getRversion()`.
This is the code accompanies the publication [Eckert RJ, Reaume A, Sturm AB, Studivan MS, and Voss JD.(in review) Symbiodiniaceae community variation among Montastraea cavernosa populations along a depth gradient on the Belize Barrier Reef](https://doi.org/10.1038/s41598-019-43479-x){target="_blank"}. Here you will find all the code to repeat the statistical analyses performed for this manuscript. All of the accompanying data can be found on my [github](https://github.com/RyanEckert/Belize_Mcav_Symbiodiniaceae_ITS2/tree/master/stats){target="_blank"}.  
...
####Additional protocols and walkthroughs:<br>

  * [Library preparation](https://ryaneckert.github.io/Belize_Mcav_Symbiodiniaceae_ITS2/lab_protocol){target="_blank"}<br>
  * [Sequence trimming and filetering](https://ryaneckert.github.io/Belize_Mcav_Symbiodiniaceae_ITS2/seq_processing){target="_blank"}<br>
  * [OTU calculation](https://ryaneckert.github.io/Belize_Mcav_Symbiodiniaceae_ITS2/dada_lulu){target="_blank"}
<br><br>
...
Here we normalize OTU counts with weighted trimmed mean of M-values (TMM; [Robinson and Oshlack 2010](https://doi.org/10.1186/gb-2010-11-3-r25){target="_blank"}). This helps to account for disparity in sequencing depth across libraries.
...
```{r, resulting alpha plot figure panel, echo = FALSE, fig.width = 7.75, fig.height = 9.25, results = 'hide'}
plot(divPlots)
```
<br><br>
...
```{r, pcoa plot, echo = FALSE, fig.width = 8, fig.height = 5}
plot(its2Pcoa)
```
<br><br>
...
```{r, nmds plot, echo = FALSE, fig.width = 8, fig.height = 5}
plot(its2NmdsPlot)
```
<br>
...
```{r, plot new nmds, echo = FALSE, fig.width = 9.2, fig.height = 5.75}
plot(its2NmdsPlot2)
```
Here we can see that there is not a larger pattern in the data that was being masked by the C3g dominant samples
<br>

##nMDS with all original ASVs
ASVs were clustered at 97% to produce OTUs for these analyses. While this is biologically relevant for Symbiodiniaceae, we can still look at the original ASVs to see if clustering masked any patterns in the data.<br>
Prep the data
```{r, asv data prep}
its2Asv = its2dada
...
its2AsvNorm$Depth = factor(its2AsvNorm$Depth, levels = c("10", "16", "25", "35"))
levels(its2AsvNorm$Depth)
its2AsvNorm$Site = factor(its2AsvNorm$Site, levels(its2AsvNorm$Site)[c(4, 2, 3, 1)])
levels(its2AsvNorm$Site)
```

```{r, asv nmds, results = 'hide'}
set.seed(694)
its2AsvNmds = metaMDS(its2AsvNorm[4:ncol(its2AsvNorm)], try = 200)
```

```{r, asv nmds plot, fig.show = 'hide'}
its2AsvNmds
...
```{r, save asv nmds, results = 'hide'}
ggsave("nMDS_ITS2_ASV.eps", plot = its2AsvNmdsPlot, width = 8, height = 5, dpi = 600,
       device = "eps")
```

```{r, plot asv nmds, echo = FALSE, fig.width = 8, fig.height = 5}
plot(its2AsvNmdsPlot)
```
...
```{r, otuplot, echo = FALSE, fig.width = 8, fig.height = 6}
plot(OTUplot)
```
<br><br>
...
###Fitting the model
Here we are using Depth as a fixed effect and running 10^5^ iterations with a 5000 rep burn-in period.
...
```{r, display glmm plot, echo = FALSE, fig.width = 8, fig.height = 6, reults = 'hide'}
plot(its2glmPlotD)
```
<br><br>

#Phylogenetic tree of *M. cavernosa* samples
***
...
##Contsrtuct phylogenetic tree
Based on genotypes from [Eckert, R. J., Studivan, M. S., and Voss, J. D. (2019). Populations of the coral species *Montastraea cavernosa* on the Belize Barrier Reef lack vertical connectivity. Sci. Rep. 9, 7200.](https://doi.org/10.1038/s41598-019-43479-x){target="_blank"} Starred samples in the data set contain the unique C3g dominant communities.
```{r, plotting phylogenetic tree, fig.show = 'hide'}
set.seed(694)
MCAVtree = CBCdist %>%
...
##Save the tree
```{r, save tree plot, results = 'hide', message = FALSE, warning = FALSE}
ggsave("MCav_tree.eps", plot(MCAVtree) %>% add.scale.bar(length = 0.05), height = 30, units = "in", dpi = 600)
```

```{r, tree plot, fig.height = 35, echo = FALSE}
plot(MCAVtree)%>%

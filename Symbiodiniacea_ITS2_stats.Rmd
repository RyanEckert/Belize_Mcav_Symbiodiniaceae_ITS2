...
#About this document
This is the code accompanies the publication **Eckert RJ, Reaume A, Sturm AB, Studivan MS, and Voss JD.(in review) Symbiodiniaceae community variation among Montastraea cavernosa populations along a depth gradient on the Belize Barrier Reef.**. Here you will find all the code to repeat the analyses performed for this manuscript. All of the accompanying data can be found on my [github](https://github.com/RyanEckert/Symbiodiniaceae-ITS2-ASV).  
...
The data used for this analysis are OTUs calculated with R packages *dada2* and *LULU*. The raw Symbiodiniaceae *ITS2* sequences obtained from *Montastraea cavernosa* samples can be found in the NCBI SRA under project number XXX <XXX>. Hopefully you are able to follow along this file and find it useful to use with your own data!  
...
```{r, environment setup, include = TRUE, message = FALSE, warning = FALSE}
if (!require("pacman")) install.packages("pacman") # for rapid install if not in library
#if(!require("pairwiseAdonis")) pacman::p_load_gh('pmartinezarbizu/pairwiseAdonis/
#                                                 pairwiseAdonis')
pacman::p_load(car, cluster, dplyr,  dunn.test, edgeR, ggplot2, ggpubr, labdsv, MCMC.OTU, 
...
####formatting the data so we can plot nicely with ggpPlot
...
Stack the OTU data and adjest our columns and factor names for plotting
```{r, otu stack}

...
kable(etbrGel, col.names = (c("Reagent","For 300 mL gel"))) %>%
  ...
  add_header_above(c("Sodium Borate/EtBr 1.5% Agarose gel Recipe"=2), line = FALSE, font_size = "Large", align = 'left') %>%
...
From [Eli Meyer](https://github.com/Eli-Meyer/ASV_utilities)
<br>
...
6. To determine volumes of each library for the combined pool:
      ...
      b. Calculate proportion of each library to sequence as:  
                $PL = 2^{[CT(sample) – CT (reference)]}$
      c. Calculate the volume of each library to use as:  
                $V = PL * X µL$  
            i. $X$ = vol of reference sample added to pool (somewhere around 20--25 µL for ITS2 libraries) 
